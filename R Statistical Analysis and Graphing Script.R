...
ggplot(Epochs, aes(x = epochs, y = percent_train_accuracy)) +
  ...
  geom_hline(yintercept = 50, color = "orange", linetype = "dashed", size = 0.7) +
  labs(x="Epochs", y="Training accuracy (%) with 100 samples")+
...
ggplot(Epochs, aes(x = epochs, y = percent_test_accuracy)) +
  ...
  geom_smooth(method="lm", se=FALSE)+
  geom_hline(yintercept = 50, color = "orange", linetype = "dashed", size = 0.7) +
  labs(x="Epochs", y="Testing accuracy (%) for 20 test samples")+
  theme(axis.title.y = element_text(size = 8), axis.title.x = element_text(size =8))
...
ggplot(BatchSize, aes(x = batch_size, y = percent_train_accuracy)) +
  ...
  labs(x="Batch Size", y="Training accuracy (%) with 100 samples")+
  theme(axis.title.y = element_text(size = 8), axis.title.x = element_text(size =8))
...
ggplot(BatchSize, aes(x = batch_size, y = percent_test_accuracy)) +
  ...
  geom_hline(yintercept = 50, color = "orange", linetype = "dashed", size = 0.7) +
  scale_x_continuous(breaks = c(5, 10, 25, 50, 75, 100))+
  labs(x="Batch Size", y="Testing accuracy (%) for 20 test samples")+
  theme(axis.title.y = element_text(size = 8), axis.title.x = element_text(size =8))
...
ggplot(freq_Batchsize, aes(x = batch_size, y = (test_accuracy*100), size = freq, fill = freq)) +
  geom_point(alpha = 0.8, shape = 21, color = "black") +
  scale_size_continuous(name = "Bubble size", range = c(1, 16), guide = "none") +
